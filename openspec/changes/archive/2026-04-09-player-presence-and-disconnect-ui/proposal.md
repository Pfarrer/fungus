...
- `multiplayer-sync`: Add a minimal server-to-client presence signal so the client can distinguish remote-player disconnects from local transport retry state

## Impact

- Client HUD and overlay flow
- Connection-status messaging and retry behavior
- Minimal multiplayer presence signaling from the server
