...
The server already keeps matches alive after disconnects and resumes play on reconnect. This change is about making that lifecycle visible and understandable in the client while keeping protocol changes as small as possible.
...
**Non-Goals:**
- New server endpoints or broad protocol changes
...
### 2. Use a minimal server presence signal for remote-player connectivity

**Decision**: Add a small server-to-client presence event that tells the remaining player when the remote player disconnects or reconnects. Keep the reconnect countdown and abandonment timer on the client.

**Rationale**: The client can infer its own transport retry state, but it cannot reliably infer when the opponent specifically left without a server signal. A single presence event keeps the protocol small while making the opponent-left state implementable.

**Alternatives considered**:
- *No protocol change*: Insufficient, because the current client only sees its own socket state and generic waiting/match-state messages.
- *Server-driven timers or richer presence snapshots*: More protocol surface area than needed for this UX.

### 3. Use the client as the source of the presence timer
...
### 4. Treat opponent disconnect as a visible in-game state
...
### 5. Escalate to abandonment only after a client timeout
...
### 6. Keep multiplayer and single-player UI paths separate
