...
Each connection between nodes SHALL have a maximum distance defined by map configuration. A player SHALL only place a new node if it is within the maximum connection distance of at least one existing friendly node.

#### Scenario: Within range
- **WHEN** max connection distance is 5 and a player places a node 4 units from their closest friendly node
- **THEN** the placement succeeds and an edge is created

#### Scenario: Out of range
- **WHEN** max connection distance is 5 and a player places a node 6 units from their closest friendly node
